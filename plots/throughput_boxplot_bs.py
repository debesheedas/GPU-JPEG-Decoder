...
if throughput_data:
    ...
    plt.gca().set_xticks(batch_sizes)  # Set specific tick positions
    plt.gca().set_xticklabels(batch_sizes, fontsize=12)  # Ensure the tick labels are visible

    plt.title("Throughput vs. Batch Size for Various JPEG Decoders", fontsize=18)
    ...
    png_path = os.path.join(base_path, 'plots', 'throughput_vs_batchsize.png')
    svg_path = os.path.join(base_path, 'plots', 'throughput_vs_batchsize.svg')
    pdf_path = os.path.join(base_path, 'plots', 'throughput_vs_batchsize.pdf')

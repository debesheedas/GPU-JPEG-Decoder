...
#include "/home/dphpc2024_jpeg_1/GPU-JPEG-Decoder/cudaH-implementation/src/parser.h"
    ...
    for (auto _ : state) {
        ...
        for (size_t batchIdx = 0; batchIdx < numBatches; ++batchIdx) {
            ...
            for (size_t i = 0; i < currentBatchSize; ++i) {
                ...
                extract(host_data->imagePath, data->quantTables, host_data->imageData, data->width, data->height, host_data->huffmanTrees);
                allocate(data->yCrCbChannels, data->rgbChannels, data->outputChannels, data->width, data->height, data->zigzagLocations);
            }
            ...
            for (size_t i = 0; i < currentBatchSize; ++i) {
                ...
                // extract(host_data->imagePath, data->quantTables, host_data->imageData, data->width, data->height, host_data->huffmanTrees);
                // allocate(data->yCrCbChannels, data->rgbChannels, data->outputChannels, data->width, data->height, data->zigzagLocations);
                performHuffmanDecoding(host_data->imageData, data->yCrCbChannels, host_data->huffmanTrees, data->width, data->height);
            }
            ...
            for (size_t i = 0; i < currentBatchSize; ++i) {
                ...
                clean(data->quantTables, data->yCrCbChannels, data->rgbChannels, data->outputChannels, data->zigzagLocations, host_data->huffmanTrees);
            }
        }

...
__global__ void colorConversionKernel2(int* luminous, int* chromRed, int* chromYel, int width, int height, int xBlocks, int yBlocks) {
    ...
    if (x < width && y < height) {
        ...
        if (x == 1 && y == 1) {
            printf("red blue green index %d %d %d %d %d", castedRed, castedBlue, castedGreen, blockIndex * 64 + pixelIndexInBlock, i);
        }
...
__global__ void colorConversionKernel(int* luminous, int* chromRed, int* chromYel, int* redOutput, int* greenOutput, int* blueOutput, int width, int height, int xBlocks, int yBlocks) {
    int threadId = threadIdx.x;  // Unique thread ID within the single block
    ...
    for (int i = threadId; i < totalPixels; i += blockDim.x * gridDim.x) {
        ...
        // // Calculate 2D coordinates (x, y) from the linear index
        // int x = i % width;
        // int y = i / width;

        // // Determine the block and pixel positions
        // int blockIndex = (y / 8) * xBlocks + (x / 8); // Index of the current 8x8 block
        // int pixelIndexInBlock = (y % 8) * 8 + (x % 8); // Position within the block
        ...
        //432251872 -315892608 166334096
        if (castedRed == 432251872 ) {
            printf("%d fhdsjfh ", i);
            //printf("red blue green index %d %d %d ", castedRed, castedBlue, castedGreen);
        }
...
__global__ void performIDCTKernel2(int* arr_l, int* arr_r, int* arr_y, double* idctTable, int validHeight, int validWidth) {
...
__global__ void performIDCTKernel(int* arr_l, int* arr_r, int* arr_y, double* idctTable, int totalPixels) {
    // // Shared memory for zigzag arrays
    int threadId = blockIdx.x * blockDim.x + threadIdx.x;
    int pixelIndex = threadId;

    //printf("block index %d %d %d",threadId);
    //printf(" hahahahahh ");
...
void JPEGParser::decode() {
    ...
    int threads = 256;
    //performIDCTKernel2<<<numBlocks, threadsPerBlock>>>(luminous, chromRed, chromYel, idctTable, 8, 8);

    std::cout << "calling " << std::endl;
   performIDCTKernel<<<1, threads>>>(luminous, chromRed, chromYel, idctTable, width*height);

    //cudaDeviceSynchronize();
    std::cout << "exuted the kenrek " << std::endl;
    ...
    // colorConversionKernel2<<<gridSize, blockSize>>>(luminous, chromYel, chromRed, width, height, xBlocks, yBlocks);
    ...
    colorConversionKernel<<<1, threads>>>(luminous, chromYel, chromRed, redOutput, greenOutput, blueOutput, width , height, xBlocks, yBlocks);

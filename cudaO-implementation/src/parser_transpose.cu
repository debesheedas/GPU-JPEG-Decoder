...
__device__ void decodeImage(uint8_t* imageData, int16_t* yCrCbChannels, int16_t* rgbChannels, int16_t* outputChannels, int width, int height, uint8_t* quantTables, uint16_t* hfCodes, int* hfLengths, int* zigzagLocations, int threadId, int blockSize) {
    ...
    __shared__ int16_t transposedBlocks[256];
    ...
    for (int i = 0; i < xBlocks*yBlocks; i++) {
        ...
        //transpose the matrix outputBlocks
        
        //idctCol(outputBlocks + ((threadId / 8) * 64 + (threadId % 8)));

        int start = (threadId / 8) * 64 + (threadId % 8);  // start column

        for (int i = 0; i < 8; i++) {            
            transposedBlocks[threadId * 8 + i] = outputBlocks[start + i * 8]; 
        }
        idctCol(transposedBlocks + (threadId / 8) * 64 + (threadId % 8) * 8);
        for (int i = 0; i < 8; i++) {
            outputBlocks[start + i * 8] = transposedBlocks[threadId * 8 + i];
        }
        ...
        //         transposedBlocks[threadId * 8 + i] = outputBlocks[start + i * 8]; 
        //     }
        //     idctCol(transposedBlocks + threadId * 8);
        //     for (int i = 0; i < 8; i++) {
        //         outputBlocks[start + i * 8] = transposedBlocks[threadId * 8 + i];
        //     }

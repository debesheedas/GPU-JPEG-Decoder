...
__global__ void decodeKernel(uint8_t* imageData, int* arr_l, int* arr_r, int* arr_y, double* idctTable, int validHeight, 
   ...
   while (pixelIndex < totalPixels) {
        ...
        if (pixelIndex % 8 == 0) {
            idctRow(zigzag_l + (blockIndex % 4) * 64 + threadIndexInBlock);
            idctRow(zigzag_r + (blockIndex % 4) * 64 + threadIndexInBlock);
            idctRow(zigzag_y + (blockIndex % 4) * 64 + threadIndexInBlock);
        }
        ...
        if (pixelIndex % 64 < 8) {
            idctCol(zigzag_l + (blockIndex % 4) * 64 + threadIndexInBlock);
            idctCol(zigzag_r + (blockIndex % 4) * 64 + threadIndexInBlock);
            idctCol(zigzag_y + (blockIndex % 4) * 64 + threadIndexInBlock);
            
        }

        __syncthreads();
        arr_l[pixelIndex] = zigzag_l[(blockIndex % 4)*64 + threadIndexInBlock];
        arr_r[pixelIndex] = zigzag_r[(blockIndex % 4)*64 + threadIndexInBlock];
        arr_y[pixelIndex] = zigzag_y[(blockIndex % 4)*64 + threadIndexInBlock];

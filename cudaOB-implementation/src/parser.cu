...
void JPEGParser::move() {
    ...
    cudaError_t err = cudaMalloc((void **)&this->scratchpad, this->guesses * 64 * sizeof(int));
    if (err != cudaSuccess) {
        std::cerr << "CUDA Error: (" << cudaGetErrorString(err) << ")" << std::endl;
        exit(EXIT_FAILURE);
    }
    err = cudaMalloc((void **)&this->predictions, this->guesses * sizeof(int));
    if (err != cudaSuccess) {
        std::cerr << "CUDA Error: (" << cudaGetErrorString(err) << ")" << std::endl;
        exit(EXIT_FAILURE);
    }
...
__device__ int entropyDecode(int threadId, int bitOffset, uint8_t* imageData, int* channel_scratchpad,
                                uint16_t* dcHfcodes, int* dcHflengths, uint16_t* acHfcodes, int* acHflengths) {
    // if (threadId == 1) printf("offset %d\n", bitOffset);
    int code = 0;
    int code_length = 0;
    int match = match_huffman_code(imageData, bitOffset, dcHfcodes, dcHflengths, code, code_length);
    // if (threadId == 1) printf("match %d\n", match);
    if (match == -1) return -1;

    bitOffset += code_length;
    uint16_t bits = getNBits(imageData, bitOffset, code);
    // if (threadId == 1) printf("bits %d\n", bits);
    int decoded = decodeNumber(code, bits); 
    // int dcCoeff = decoded + oldCoeff;
    //outBuffer[0] = dcCoeff * (int) quant[0];
    channel_scratchpad[0] = decoded;
    // if (threadId == 1) printf("channel %d\n", channel_scratchpad[threadId]);
    // if (threadId == 1) printf("hi\n");
    int length = 1;
    while (length < 64) {
        match = match_huffman_code(imageData, bitOffset, acHfcodes, acHflengths, code, code_length);
        if (match == -1) return -1;
        ...
        if (length < 64) {
            ...
            //outBuffer[length] = val;
            // if (threadId == 1) printf("hi\n");
            channel_scratchpad[length] = decoded;
            // if (threadId == 1) printf("hi-end\n");
            length++;
        }
    }
    // Update oldCoeff for the next MCU
    // oldCoeff = dcCoeff;
    // if (threadId == 1) printf("returning\n");
    return bitOffset;
}
...
__device__ void parallelBuildMCU(int threadId, uint8_t* imageData, int* arr_l, int* arr_r, int* arr_y,
                                       uint16_t* hf0codes, int* hf0lengths, uint16_t* hf16codes, int* hf16lengths,
                                       uint16_t* hf1codes, int* hf1lengths, uint16_t* hf17codes, int* hf17lengths,
                                       int nBlocks, int guesses, int* predictions, int* scratchpad) {
    __shared__ int red_found;
    __shared__ int red_pos;
    __shared__ int yellow_pos;
    __shared__ int yellow_found;
    ...
    while (blockCounter < nBlocks) {
        ...
        if (threadId + x == red_pos) {
            red_found = 1;
            yellow_pos = predictions[threadId];
        ...
        if (threadId + x == yellow_pos) {
            ...
            next_offset = predictions[threadId];
        }
        if (threadId == 0 && red_found == 0) { // red channel was not correctly decoded so decode red 
        ...
        __syncthreads();
        if (threadId == 0) {
            // add old coefficients
            if (blockCounter != 0) {
                arr_l[64*blockCounter] += arr_l[64*(blockCounter-1)];
                arr_r[64*blockCounter] += arr_r[64*(blockCounter-1)];
...
__global__ void decodeKernel(uint8_t* imageData, int* arr_l, int* arr_r, int* arr_y, double* idctTable, int validHeight, 
    ...
    while (pixelIndex < totalPixels) {
        ...
        performZigzagReordering(arr_l, arr_r, arr_y, zigzag_l, zigzag_r, zigzag_y, quant1, quant2, 
...
void JPEGParser::decode() {
    ...
    decodeKernel<<<1, this->guesses>>>(this->imageData, this->luminous, this->chromRed, this->chromYel, idctTable, 8, 8,  
                                            ...
                                            this->hf0lengths, this->hf1lengths, this->hf16lengths, this->hf17lengths, this->scratchpad, this->guesses, this->predictions);
...
void JPEGParser::write() {
    ...
    fs::path output_dir = "../testing/cudaOB_output_arrays"; // Change the directory name here for future CUDA implementations

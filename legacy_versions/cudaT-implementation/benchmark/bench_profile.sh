...
TMPDIR="$TMPDIR" /usr/local/cuda/bin/ncu --set full --nvtx --target-processes all \
    -f --export ../report_runtime_bench.ncu-rep ./benchmark_test --benchmark_out=benchmark_results.json --benchmark_out_format=json

if [ $? -eq 0 ]; then
    echo "Profiling complete. Results saved to 'report.ncu-rep'."
else

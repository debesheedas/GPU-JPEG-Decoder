...
// Function to log current CUDA memory information
...
void allocate(uint16_t*& hfCodes, int*& hfLengths, std::unordered_map<int,HuffmanTree*>& huffmanTrees, 
              int16_t*& yCrCbChannels, int16_t*& rgbChannels, int16_t*& outputChannels, int width, int height, int*& zigzagLocations) {
    try {
        ...
        // Copy Huffman table data to the GPU
        int index = 0;
        for (int i = 0; i < 4; ++i) {
            if (i > 1 && index < 16)
                index = 16;

            checkCudaError(cudaMemcpy(hfCodes + i * 256, huffmanTrees[index]->codes, codeSize, cudaMemcpyHostToDevice), "Failed to copy Huffman codes to device");
            cudaDeviceSynchronize();
            checkCudaError(cudaMemcpy(hfLengths + i * 256, huffmanTrees[index]->codeLengths, lengthSize, cudaMemcpyHostToDevice), "Failed to copy Huffman lengths to device");
            cudaDeviceSynchronize();
            index++;
        }
        ...
        std::cerr << "Error during memory allocation: " << e.what() << std::endl;
        exit(EXIT_FAILURE);
    }
}
    ...
    try {
        ...
        while (true) {
            ...
            if (marker == MARKERS[0]) {
                ...
                checkCudaError(cudaMalloc((void**)&quantTables, 128 * sizeof(uint8_t)), "Failed to allocate device memory for quantization table.");
                cudaDeviceSynchronize();

                std::vector<uint8_t> hostQuantTable(64);
                stream.getNBytes(hostQuantTable, 64);

                checkCudaError(cudaMemcpy(quantTables, hostQuantTable.data(), 64 * sizeof(uint8_t), cudaMemcpyHostToDevice), "Failed to copy quantization table to device.");
                cudaDeviceSynchronize();
                ...
                if (stream.getMarker() == MARKERS[2]) {
                    ...
                    stream.getNBytes(hostQuantTable, 64);
                    checkCudaError(cudaMemcpy(quantTables + 64, hostQuantTable.data(), 64 * sizeof(uint8_t), cudaMemcpyHostToDevice), "Failed to copy second quantization table to device.");
                    cudaDeviceSynchronize();
                } else {
                    std::cerr << "Error parsing second quantization table." << std::endl;
                    exit(EXIT_FAILURE);
                }
                ...
                imageDataLength--; // Remove the trailing byte (0xff)
                checkCudaError(cudaMalloc((void**)&imageData, imageDataLength * sizeof(uint8_t)), "Failed to allocate device memory for image data.");
                cudaDeviceSynchronize();
                checkCudaError(cudaMemcpy(imageData, hostImageData.data(), imageDataLength * sizeof(uint8_t), cudaMemcpyHostToDevice), "Failed to copy image data to device.");
                cudaDeviceSynchronize();
                break;
            }
        }

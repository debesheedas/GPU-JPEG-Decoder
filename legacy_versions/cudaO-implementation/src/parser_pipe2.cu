...
__device__ void decodeImage(uint8_t* imageData, int16_t* yCrCbChannels, int16_t* rgbChannels, int16_t* outputChannels, int width, int height, uint8_t* quantTables, uint16_t* hfCodes, int* hfLengths, int* zigzagLocations, int threadId, int blockSize) {
    ...
    __shared__ int16_t inputBlocks1[192]; // Buffer 0
    __shared__ int16_t inputBlocks2[192]; // Buffer 1
    __shared__ volatile int bufferReadyToProcess[64]; 
    __shared__ volatile int bufferReadyToWrite[64];
    ...
    bufferReadyToProcess[threadId] = 0; 
    bufferReadyToWrite[threadId] = 1;

    int warpId = threadId / 32;
    int laneId = threadId % 32;
    
    for (int i = 0; i <= xBlocks*yBlocks; i++) {
        int activeDecode = i % 2; // if i is even we write to left

        // The first warp decodes
        if (warpId == 0) {
            if (threadId == 0) {
                int16_t* curStart = (activeDecode == 0) ? inputBlocks1 : inputBlocks2;

                while (bufferReadyToWrite[activeDecode] == 0) {
                    // Wait for the buffer to be free for writing
                }
                ...
                atomicExch((int*)&bufferReadyToProcess[activeDecode], 1);
                atomicExch((int*)&bufferReadyToWrite[activeDecode], 0);

            }
        // The second warp idct + color
        } else {
            int16_t* curStart = (activeDecode == 0) ? inputBlocks1 : inputBlocks2;

            while (bufferReadyToProcess[activeDecode] == 0) {
                // Wait for the buffer to be ready for processing
            }

            pixelIndex = laneId;
            while (pixelIndex < 64) {
                performZigzagReordering(curStart, outputBlocks, quantTables, 0, pixelIndex, pixelIndex, zigzagMap);
                performZigzagReordering(curStart, outputBlocks, quantTables,
                                    1, pixelIndex + 64, pixelIndex, zigzagMap);
                performZigzagReordering(curStart, outputBlocks, quantTables,
                                    2, pixelIndex + 128, pixelIndex, zigzagMap);
                pixelIndex += 32;
            }
            idctRow(outputBlocks + (laneId / 8) * 64 + (laneId % 8) * 8);
            idctCol(outputBlocks + ((laneId / 8) * 64 + (laneId % 8)));
            ...
            while(pixelIndex < 192){
                curStart[pixelIndex] = 0;
                pixelIndex += 32;
            }

            atomicExch((int*)&bufferReadyToWrite[activeDecode], 1);   // Mark buffer as ready to write
            atomicExch((int*)&bufferReadyToProcess[activeDecode], 0);
        }
        // __syncthreads();
    }
}

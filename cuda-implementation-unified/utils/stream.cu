...
__device__ Stream::Stream(uint8_t* data) {
    this->data = data;
    this->position = 0;
}

__device__ uint8_t Stream::getBit() {
...
__device__ uint16_t Stream::getNBits(int n) {
    int curVal = 0;
    for (int i = 0; i < n; i++) {
        curVal = curVal * 2;
        curVal += this->getBit();
...
__device__ uint8_t Stream::getByte() {
    uint8_t curVal = this->data[this->position >> 3];
    this->position += 8;
    return curVal;
}

__device__ uint16_t Stream::getMarker() {
...
__device__ void Stream::getNBytes(uint8_t* arr, int length) {
    for (int i = 0; i < length; i++) {
        arr[i] = this->getByte();
    }
}

__device__ int Stream::decodeNumber(uint8_t code, int bits) {

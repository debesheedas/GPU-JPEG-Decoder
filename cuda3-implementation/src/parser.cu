...
void JPEGParser::buildMCU(int* deviceBuffer, Stream* imageStream, int hf, int quant, int& oldCoeff, int validWidth = 8, int validHeight = 8) {
    ...
    oldCoeff = dcCoeff;
    cudaMemcpy(deviceBuffer, hostBuffer.data(), 64 * sizeof(int), cudaMemcpyHostToDevice);

    

}
void JPEGParser::blockIDCT(int* hostBuffer, int validWidth = 8, int validHeight = 8) {
    // Create and process the IDCT for this block with the valid dimensions
    
    // cudaMemcpy(hostBuffer, hostBuffer.data(), 64*sizeof(int), cudaMemcpyHostToDevice);
    IDCT* idct = new IDCT(hostBuffer, idctTable);
...
void JPEGParser::decode() {
    ...
    curLuminous = luminous;
    curChromRed = chromRed;
    curChromYel = chromYel;
    ...
    for (int y = 0; y < yBlocks; y++) {
        for (int x = 0; x < xBlocks; x++) {
            ...
            this->blockIDCT(curLuminous, blockWidth, blockHeight);
            this->blockIDCT(curChromRed, blockWidth, blockHeight);
            this->blockIDCT(curChromYel, blockWidth, blockHeight);
            curLuminous += 64;
            curChromRed += 64;
            curChromYel += 64;
        }
...
void JPEGParser::write() {
    ...
    fs::path output_dir = "../testing/cuda3_output_arrays"; // Change the directory name here for future CUDA implementations

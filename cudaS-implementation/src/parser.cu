...
__device__ int16_t clip(int16_t value) {
    // Use min and max operations to bound the value
    value = max(-256, value);
    value = min(255, value);
    return value;
}
...
__device__ void idctCol(int16_t* block) {
    ...
    x0 = (block[0] << 8) + 8192; // Scale DC coefficient
    x1 = block[4] << 8;
    x2 = block[6];
    x3 = block[2];
    x4 = block[1];
    ...
    block[0] = clip((x7 + x1) >> 14);
    block[1] = clip((x3 + x2) >> 14);
    block[2] = clip((x0 + x4) >> 14);
    block[3] = clip((x8 + x6) >> 14);
    block[4] = clip((x8 - x6) >> 14);
    block[5] = clip((x0 - x4) >> 14);
    block[6] = clip((x3 - x2) >> 14);
    block[7] = clip((x7 - x1) >> 14);
}
...
__device__ void decodeImage(uint8_t* imageData, int16_t* yCrCbChannels, int16_t* rgbChannels, int16_t* outputChannels, int width, int height, uint8_t* quantTables, uint16_t* hfCodes, int* hfLengths, int* zigzagLocations, int threadId, int blockSize) {
    ...
    while (pixelIndex * 8 < totalPixels) {  // load memory into shared
        int start = (pixelIndex / 8) * 64 + (pixelIndex % 8);

        for (int i = 0; i < 8; i++) {            
            sharedMem[threadId * 8 + i] = rgbChannels[start + i * 8]; 
        }
        idctCol(sharedMem + threadId * 8);
         for (int i = 0; i < 8; i++) {
            rgbChannels[start + i * 8] = sharedMem[threadId * 8 + i];
        }
        pixelIndex += blockSize;
    }
    pixelIndex = threadId;
    ...
    while (pixelIndex * 8 < totalPixels) {  // load memory into shared
        ...
        for (int i = 0; i < 8; i++) {            
            sharedMem[threadId * 8 + i] = rgbChannels[start + totalPixels + i * 8]; 
        }
        idctCol(sharedMem + threadId * 8);
         for (int i = 0; i < 8; i++) {
            rgbChannels[start + totalPixels + i * 8] = sharedMem[threadId * 8 + i];
        }
        pixelIndex += blockSize;
    }
    pixelIndex = threadId;
    ...
    while (pixelIndex * 8 < totalPixels) {  // load memory into shared
        ...
        for (int i = 0; i < 8; i++) {            
            sharedMem[threadId * 8 + i] = rgbChannels[start + 2 * totalPixels + i * 8]; 
        }
        idctCol(sharedMem + threadId * 8);
         for (int i = 0; i < 8; i++) {
            rgbChannels[start + 2* totalPixels + i * 8] = sharedMem[threadId * 8 + i];
        }
        pixelIndex += blockSize;
    }
    pixelIndex = threadId;
...
void write(int16_t* outputChannels, int width, int height, std::string filename) {
    ...
    fs::path output_dir = "../testing/cudaS_output_arrays";

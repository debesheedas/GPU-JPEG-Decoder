...
std::string path_to_decoder = "/home/dphpc2024_jpeg_1/cfernand/GPU-JPEG-Decoder/nvjpeg-implementation/nvJPEG/nvJPEG-Decoder/build/nvjpegDecoder";
...
void JPEGDecoderBenchmark(benchmark::State& state, const std::vector<std::string>& imagePaths) {
    ...
    for (auto _ : state) {
        ...
        std::string command = path_to_decoder + " -fmt rgb -b 1 -i " + imagePath + " > /dev/null 2>&1";

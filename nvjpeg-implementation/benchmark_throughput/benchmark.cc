...
std::string path_to_decoder = "/home/dphpc2024_jpeg_1/cfernand/GPU-JPEG-Decoder/nvjpeg-implementation/nvJPEG-Decoder-MultipleInstances/build/nvJPEGDecMultipleInstances";
...
void JPEGDecoderBenchmark(benchmark::State& state, const std::vector<std::string>& imagePaths, const std::string datasetPath) {
    ...
    for (auto _ : state) {
        ...
        std::string command = path_to_decoder + " -fmt rgb -b 1000 -t 32 -i " + datasetPath + " > /dev/null 2>&1";
...
int main(int argc, char** argv) {
    // std::string datasetPath = "/home/dphpc2024_jpeg_1/cfernand/GPU-JPEG-Decoder/nvjpeg-implementation/benchmark_throughput/bh/";
